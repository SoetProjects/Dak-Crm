...
export default async function QuotePrintPage({ params }: Props) {
  ...
  const { company, customer, lines } = quote;

  const quoteDate = quote.createdAt.toLocaleDateString("nl-NL");
    ...
    <>
      ...
      <div className="no-print flex items-center gap-3 border-b border-slate-200 bg-white px-6 py-3 shadow-sm">
        ...
        <p className="ml-auto text-xs text-slate-400">
          Gebruik "Opslaan als PDF" in het printvenster om een PDF te genereren.
        </p>
      </div>
      ...
      <div className="print-page mx-auto max-w-3xl px-8 py-10 text-sm text-slate-900">
        ...
        <div className="flex justify-between gap-6">
          ...
          <div className="text-right">
            <p className="text-2xl font-bold text-[#1b3a6b]">OFFERTE</p>
            <p className="mt-2 font-mono text-base font-semibold">{quote.quoteNumber}</p>
            <div className="mt-3 space-y-1 text-xs text-slate-500">
              <p>Datum: <span className="text-slate-800">{quoteDate}</span></p>
              {validUntil && <p>Geldig tot: <span className="text-slate-800">{validUntil}</span></p>}
              <p>
                Status:{" "}
        ...
        {/* Subject */}
        <div className="mb-6">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Onderwerp</p>
          <p className="mt-1 font-medium text-slate-900">{quote.title}</p>
        </div>
        ...
        {lines.length > 0 ? (
          ...
          <p className="text-slate-400 italic">Geen regels toegevoegd.</p>
        )}
